...
def process_file(filename, fields):
    ...
    class_fields = ['family','class','phylum','order','kingdom','genus']
    data = []
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            for field in process_fields:
                if field == "rdf-schema#label" and line[field].endswith(')'):
                    line[field] = line[field][:line[field].find('(')].strip()
                elif field == 'name':
                    line[field] = line["rdf-schema#label"]
                elif line[field] == 'NULL':
                    line[field] = None
                elif field == 'synonym':
                    line[field] = parse_array(line[field].replace('*',''))
                else:
                    line[field] = line[field].strip()
                
                if fields[field] in class_fields:
                    teste['classification'][fields[field]]=line[field]
                else:
                    teste[fields[field]]=line[field]
